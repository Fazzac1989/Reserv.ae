...
select 'venues visible (expect all live) : ' || count(*) || ' of ' || (select count(*) from public.venues) from public.venues;
select 'venue policies (one per venue)   : ' || count(*) || ' of ' || (select count(*) from public.venues) from public.venue_policies;
...
select 'venue CONTACTS (one per venue)   : ' || count(*) || ' of ' || (select count(*) from public.venues) from public.venue_contacts;
select 'events_log (expect >0)           : ' || count(*) from public.events_log;
